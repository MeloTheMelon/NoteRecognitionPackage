...
public class NoteRecognizer : MonoBehaviour {
    ...
    */
    ...
    public static float baseNoteFrequency = 82.41f;
    ...
    public void specturmToNote(float[] spectrum, int sampleRate)
    {
        if (HPS) spectrum = harmonicProductSpectrum(spectrum);
            ...
            if (max > recognizeNoteVolumeThreshold)
            {
                 float freq = max * (sampleRate / 2) / (spectrum.Length);  //Calculate the frequency of the index from the local maximum
                    if (freq > 75.0f)                                       //Lower values are just noise, so they can be ignored, the lowest possible played Frequence would be 82Hz
                    {
                        ...
                        Debug.Log("" + currentNote.noteName + currentNote.octave);
                    }
                    else
                    {
    ...
    //Check for local max for a specific note
    bool getLocalMaxIndexForSpecificFreq(float[] spectrum, float freq, int sampleRate)
    {
        float volume = getVolume(spectrum);
        int index = (int) (2 * freq * spectrum.Length / sampleRate);
        if (spectrum[index] > recognizeNoteVolumeThreshold* 0.10f && spectrum[index - 1] < spectrum[index] && spectrum[index + 1] < spectrum[index]) return true;
        else return false;
    }
    ...
    public bool compareNotes(Note a, Note b)
    {
...
public class Note
{
    ...
    public float noteToFrequency()
    {
        int halfsteps = this.noteToHalfsteps();
        float freq = NoteRecognizer.baseNoteFrequency;
        return freq * Mathf.Pow(2.0f, (halfsteps / 12.0f));
    }
